...
#def find_in_page(portfolio_Only, wrapper, ):
#    i = 0    
#    print len(portfolio)
#    for p in portfolio:
...
def get_rank_and_dateLastTrade(UserName,portfolio): #method will get rank form users page
    i = 0    
    print len(portfolio)
    for p in portfolio: #looking at all the html from the portfolio page stil in Bsoup form
        r = requests.get(str(p)) 
        soup = BeautifulSoup(r.content)
        for port in soup.find_all("div"): # looking at all "div" tags inside of portfolio page
            s = str(port) # convert the div tags to a string
            print "----------------------"
            #print port
            print "---------------------"
    #            if "smallertext" in s:awew
    #                if "Traded" in s:
    #                    #print port.text
    #                    #print s
    #                    x = s.find("Traded")
            ...
            if "smalltext" in s:
                if "Ranked" in s:
                    ...
                    if (Rank < 3000):
                        ...
                        send_to_dataBase("Users","UserName","\'"+UserName[i]+"\'") # thisis going to Uses database and must be done first
                        ...
                        Data = [str(rows[0]),str(Rank)] #rows[0] is the name of the trader I think
                        print Data
                        print Rank
                        ##Column = ["UserId","Rank"]
                        #3Data = [fk,Rank]
